...
import { pickProjectFile } from "@/lib/projects-utils";
...
export default function ProjectsPage() {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const created = importProject(data);
      if (!created) {
        setImportError("That file doesn't look like a Drishti project.");
      }
    } catch (err: any) {
  ...
  return (
    <div className="space-y-8">
      {/* Hero header — matches the setup page style */}
      <motion.header
      ...
      >
        ...
        <p className="mt-3 max-w-2xl text-sm text-muted md:text-base">
          Group components, run consistent selections, and surface cross-component
          issues like galvanic pairs or procurement consolidation.
        </p>
        ...
        {importError && (
          <p className="mt-3 text-xs text-amber-500">{importError}</p>
        )}
      </motion.header>
    ...
    >
      <Card className="text-center">
        ...
        <p className="mx-auto mt-2 max-w-md text-sm text-muted">
          ...
          and get consistent material picks across all of them, with warnings
          about galvanic pairs and procurement consolidation.
        </p>
